 ...
 * @dev Adapted to be a library instead of a contract
 */
// slither-disable-next-line missing-inheritance
library BN256G2 {
    error PointNotOnCurve();
    error ModularInverseError();
    ...
    function ecTwistAdd(
        ...
        if (pt1xx == 0 && pt1xy == 0 && pt1yx == 0 && pt1yy == 0) {
            if (!(pt2xx == 0 && pt2xy == 0 && pt2yx == 0 && pt2yy == 0)) {
                if (!_isOnCurve(pt2xx, pt2xy, pt2yx, pt2yy)) revert PointNotOnCurve();
            }
            return (pt2xx, pt2xy, pt2yx, pt2yy);
        } else if (pt2xx == 0 && pt2xy == 0 && pt2yx == 0 && pt2yy == 0) {
            if (!_isOnCurve(pt1xx, pt1xy, pt1yx, pt1yy)) revert PointNotOnCurve();
