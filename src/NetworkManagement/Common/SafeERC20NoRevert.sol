...
library SafeERC20NoRevert {
     ...
     */
    function safeTransferNoRevert(IERC20 token, address to, uint256 value) internal returns (bool) {
    ...
    function _isReturnDataBooleanTrue(bytes memory returndata) private pure returns (bool) {
        ...
        assembly {
            returndataWord := mload(add(returndata, 32))
        }
        return returndataWord == bytes32(uint256(0x01));
    }
}