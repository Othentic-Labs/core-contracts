...
contract AttestationCenterPausable is PausableFlows {
    // INITIALIZER
    function __AttestationCenterPausable_init(
    ...
    function _grantAttestationCenterRoles(
        address _avsGovernanceMultisigOwner,
        address _operationsMultisig,
        address _communityMultisig
    ) private {
        _getPausableFlowsStorage().flowsPauseStates[PauserRolesLibrary.BATCH_PAYMENT_REQUEST_FLOW] = true;
        _getPausableFlowsStorage().flowsPauseStates[PauserRolesLibrary.EIGEN_PAYMENT_REQUEST_FLOW] = true;

        _grantRole(PauserRolesLibrary.BATCH_PAYMENT_REQUEST_FLOW, _avsGovernanceMultisigOwner);
        _grantRole(PauserRolesLibrary.EIGEN_PAYMENT_REQUEST_FLOW, _avsGovernanceMultisigOwner);
